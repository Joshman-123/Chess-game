    ...
    /*          Default Grid Dimension of Open GL 
      ...
      (-1,-1)----------------(0,-1)----------------(1,-1)
        ...
        */
        ...
        this->indices.resize(totalIndices);

        int index = 0;
        for (int i = 0; i < numSquares; ++i)
        {
            unsigned int baseVertex = i * verticesPerSquare;
            this->indices[index++] = baseVertex + 0; // Top-left
            this->indices[index++] = baseVertex + 1; // Top-right
            this->indices[index++] = baseVertex + 3; // Bottom-left

            this->indices[index++] = baseVertex + 1; // Top-right
            this->indices[index++] = baseVertex + 2; // Bottom-right
            this->indices[index++] = baseVertex + 3; // Bottom-left
        }
